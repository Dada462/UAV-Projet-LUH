...
class RobotModeState():
    def __init__(self):
        ...
        global HOVER, LAND, FOLLOWPATH, PILOT_TAKEOVER, OA
        global INIT_LAND, INIT_FOLLOWPATH, INIT_HOVER

        # States
        INIT, LANDED, HOVERING, CONTROL, PILOT, TAKEOFF, STOPPING, LANDING, ATCP = 'INIT', 'LANDED', 'HOVERING', 'CONTROL', 'PILOT', 'TAKEOFF', 'STOPPING', 'LANDING', 'ATCP'
        # Modes
            ...
            if self.state == INIT:
                ...
                if self.mode != expectedMode or self.mode == LOITER:
                    ...
                    if self.speed >= 0.1:
                        ...
                        if self.landAccepted and self.mode == LAND:
                            self.state = LANDING
                    elif self.speed < 0.1 and self.userInput == OA:
                        self.state=ATCP
                    elif self.speed < 0.1:
                        self.state = HOVERING
