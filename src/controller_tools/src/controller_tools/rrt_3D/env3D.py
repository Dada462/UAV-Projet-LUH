...
def getballs():
    spheres = [[2.0,6.0,2.5,1.0],[8,8.0,1.5,5]]
    Obstacles = []
    for i in spheres:
        Obstacles.append([j for j in i])
    return np.array(Obstacles)
...
class env():
    def __init__(self, xmin=0, ymin=0, zmin=0, xmax=20, ymax=20, zmax=10, resolution=1,start=np.array([2.0, 2.0, 2.0]),goal=np.array([12.0, 16.0, 0.5])):
    # def __init__(self, xmin=-5, ymin=0, zmin=-5, xmax=10, ymax=5, zmax=10, resolution=1): 
        ...
        self.balls = getballs()
        # self.OBB = np.array([obb([5.0,7.0,2.5],[0.5,2.0,2.5],R_matrix(130,0,0)),
