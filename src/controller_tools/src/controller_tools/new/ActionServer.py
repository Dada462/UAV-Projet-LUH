...
class ActionServer():
    ...
    def LandExecute_cb(self,goal):
        ...
        t0=time()
        if self.landing_successful==False:
        ...
        while self.landing_successful==None:
            print('LANDING WAITING FOR RESPONSE')
            print(self.landing_successful)
            if self.landing_successful==True:
                break
            elif self.landing_successful==False or self.accept_landing==False:
                result.result = result.UNKNOWN_ERROR
                self.LandServer.set_aborted(result)
                self.landing_successful=None
                self.accept_landing=True
                print('LANDING problem here',self.landing_successful,self.accept_landing)
                return
            if (time()-t0)>2:
                print('timeout')
