...
class PFController():
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.sm.state=='CONTROL' and self.sm.userInput!='HOME' and self.sm.userInput!='WAIT' and self.pathIsComputed:
                ...
                # command.type_mask = PositionTarget.IGNORE_AFX+PositionTarget.IGNORE_AFY
                command.acceleration_or_force=Vector3(*u)
                accel_command_pub.publish(command)
                ############################## Acceleration Topic ##############################
                
                # msg = AttitudeTarget()
                # msg.type_mask=AttitudeTarget.IGNORE_ATTITUDE+AttitudeTarget.IGNORE_THRUST*0
                # msg.thrust=self.displayer.values[-1]
                # # msg.thrust=u
    ...
    def LPF_control_v5(self):
        ...
        dz=Vr[2]
        T,zmin,zmax,k0,k1=self.displayer.values
        # T=0.25
        zd=zmax*(np.sin(pi*(time()-self.t0)/T)+1)+zmin
        thrust=0.5+k0*(zd-z)+k1*(-dz)
        # thrust=zd
        
        self.p.plot(time()-self.t0,dz,'z','#f5300d')
        # self.p.plot(time()-self.t0,zd,'zd','#49f50d')
        # self.p.plot(time()-self.t0,thrust,'thrust','#0dd5f5')
